...
Most aircrafts have equipped with an ADB-S transmitter, with which they send
information about their status and the position continuously, to guarantee
...
absent, it is not possible to detect complete airplanes informations.
...
need to use a cluster to perform analysis on the data. However, before executing
on the cluster, it is essential to be able to try the algorithms locally on a
small subset of data, in order to test their correctness end efficiency, and to
obtain useful statistics. For this reason, we used different technologies
depending on where we ran the analysis.
...
the strongly typing and the functional programming approach. In addition to
...
We were able to associate an airline to almost all of the flights the resulted
